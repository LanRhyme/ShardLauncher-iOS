...
$(error This platform is not currently supported for building ShardLauncher.)
endif
...
POJAV_BUNDLE_DIR      ?= $(OUTPUTDIR)/ShardLauncher.app
...
METHOD_PACKAGE = \
	...
	if [ '$(SLIMMED_ONLY)' = '0' ]; then \
		zip --symlinks -r $(OUTPUTDIR)/com.lanrhyme.shardlauncher-$(VERSION)-$(PLATFORM_NAME)$$IPA_SUFFIX Payload; \
	fi; \
	if [ '$(SLIMMED)' = '1' ] || [ '$(SLIMMED_ONLY)' = '1' ]; then \
		zip --symlinks -r $(OUTPUTDIR)/com.lanrhyme.shardlauncher.slimmed-$(VERSION)-$(PLATFORM_NAME)$$IPA_SUFFIX Payload --exclude='Payload/ShardLauncher.app/java_runtimes/*'; \
	fi

# Function to download and unpack Java runtimes.
METHOD_JAVA_UNPACK = \
	cd $(SOURCEDIR)/depends; \
	if [ ! -f "java-$(1)-openjdk/release" ] && [ -z "$$(find . -maxdepth 1 -name 'jre$(1)-*.tar.xz' -print -quit)" ]; then \
		if [ "$(RUNNER)" != "1" ]; then \
...
help:
	echo 'Makefile to compile ShardLauncher'
	echo ''
	echo 'Usage:'
	...
	echo '    make payload                        Makes Payload/ShardLauncher.app'
	echo '    make package                        Builds ipa of ShardLauncher'
...
native:
	echo '[ShardLauncher v$(VERSION)] native - start'
	mkdir -p $(WORKINGDIR)
	cd $(WORKINGDIR) && cmake . \
	...
	#	--target awt_headless awt_xawt libOSMesaOverride.dylib tinygl4angle ShardLauncher
	rm $(WORKINGDIR)/libawt_headless.dylib
	echo '[ShardLauncher v$(VERSION)] native - end'

java:
	echo '[ShardLauncher v$(VERSION)] java - start'
	$(MAKE) -C JavaApp -j$(JOBS) BOOTJDK=$(BOOTJDK)
	echo '[ShardLauncher v$(VERSION)] java - end'

jre: native
	echo '[ShardLauncher v$(VERSION)] jre - start'
	...
	echo '[ShardLauncher v$(VERSION)] jre - end'

assets:
	echo '[ShardLauncher v$(VERSION)] assets - start'
	if [ '$(IOS)' = '0' ] && [ '$(DETECTPLAT)' = 'Darwin' ]; then \
		mkdir -p $(WORKINGDIR)/ShardLauncher.app/Base.lproj; \
		...
		echo 'Due to the required tools not being available, you cannot compile the extras for ShardLauncher with an iOS device.'; \
	fi
	echo '[ShardLauncher v$(VERSION)] assets - end'

payload: native java jre assets
	echo '[ShardLauncher v$(VERSION)] payload - start'
	$(call METHOD_DIRCHECK,$(WORKINGDIR)/ShardLauncher.app/libs)
	$(call METHOD_DIRCHECK,$(WORKINGDIR)/ShardLauncher.app/libs_caciocavallo)
	$(call METHOD_DIRCHECK,$(WORKINGDIR)/ShardLauncher.app/libs_caciocavallo17)
	cp -R $(SOURCEDIR)/Natives/resources/en.lproj/LaunchScreen.storyboardc $(WORKINGDIR)/ShardLauncher.app/Base.lproj/ || exit 1
	cp -R $(SOURCEDIR)/Natives/resources/* $(WORKINGDIR)/ShardLauncher.app/ || exit 1
	cp $(WORKINGDIR)/*.dylib $(WORKINGDIR)/ShardLauncher.app/Frameworks/ || exit 1
	cp -R $(SOURCEDIR)/JavaApp/libs/others/* $(WORKINGDIR)/ShardLauncher.app/libs/ || exit 1
	cp $(SOURCEDIR)/JavaApp/build/*.jar $(WORKINGDIR)/ShardLauncher.app/libs/ || exit 1
	cp -R $(SOURCEDIR)/JavaApp/libs/caciocavallo/* $(WORKINGDIR)/ShardLauncher.app/libs_caciocavallo || exit 1
	cp -R $(SOURCEDIR)/JavaApp/libs/caciocavallo17/* $(WORKINGDIR)/ShardLauncher.app/libs_caciocavallo17 || exit 1
	$(call METHOD_DIRCHECK,$(OUTPUTDIR)/Payload)
	cp -R $(WORKINGDIR)/ShardLauncher.app $(OUTPUTDIR)/Payload
	if [ '$(SLIMMED_ONLY)' != '1' ]; then \
		cp -R $(OUTPUTDIR)/java_runtimes $(OUTPUTDIR)/Payload/ShardLauncher.app; \
	fi
	ldid -S $(OUTPUTDIR)/Payload/ShardLauncher.app; \
	if [ '$(TROLLSTORE_JIT_ENT)' == '1' ]; then \
		ldid -S$(SOURCEDIR)/entitlements.trollstore.xml $(OUTPUTDIR)/Payload/ShardLauncher.app/ShardLauncher; \
	elif [ '$(PLATFORM)' == '6' ]; then \
		ldid -S$(SOURCEDIR)/entitlements.codesign.xml $(OUTPUTDIR)/Payload/ShardLauncher.app/ShardLauncher; \
	else \
		ldid -S$(SOURCEDIR)/entitlements.sideload.xml $(OUTPUTDIR)/Payload/ShardLauncher.app/ShardLauncher; \
	fi
	chmod -R 755 $(OUTPUTDIR)/Payload
	if [ '$(PLATFORM)' != '2' ]; then \
		$(call METHOD_MACHO,$(OUTPUTDIR)/Payload/ShardLauncher.app,$(call METHOD_CHANGE_PLAT,$(PLATFORM),$$file)); \
		$(call METHOD_MACHO,$(OUTPUTDIR)/java_runtimes,$(call METHOD_CHANGE_PLAT,$(PLATFORM),$$file)); \
	fi
	echo '[ShardLauncher v$(VERSION)] payload - end'

deploy:
	echo '[ShardLauncher v$(VERSION)] deploy - start'
	cd $(OUTPUTDIR); \
	if [ '$(IOS)' = '1' ]; then \
		ldid -S $(WORKINGDIR)/ShardLauncher.app || exit 1; \
		ldid -S$(SOURCEDIR)/entitlements.trollstore.xml $(WORKINGDIR)/ShardLauncher.app/ShardLauncher || exit 1; \
		sudo mv $(WORKINGDIR)/*.dylib $(PREFIX)Applications/ShardLauncher.app/Frameworks/ || exit 1; \
		sudo mv $(WORKINGDIR)/ShardLauncher.app/ShardLauncher $(PREFIX)Applications/ShardLauncher.app/ShardLauncher || exit 1; \
		sudo mv $(SOURCEDIR)/JavaApp/build/*.jar $(PREFIX)Applications/ShardLauncher.app/libs/ || exit 1; \
		cd $(PREFIX)Applications/ShardLauncher.app/Frameworks || exit 1; \
		sudo chown -R 501:501 $(PREFIX)Applications/ShardLauncher.app/* || exit 1; \
		...
		if [ '$(PLATFORM)' != '2' ] || [ '$(TEAMID)' = '-1' ] || [ '$(SIGNING_TEAMID)' = '-1' ] || [ '$(PROVISIONING)' = '-1' ]; then \
			...
			$(call METHOD_PACKAGE); \
			if [ '$(SLIMMED_ONLY)' = '0' ]; then \
				open $(OUTPUTDIR)/com.lanrhyme.shardlauncher-$(VERSION)-$(PLATFORM_NAME).ipa; \
			else \
				open $(OUTPUTDIR)/com.lanrhyme.shardlauncher.slimmed-$(VERSION)-$(PLATFORM_NAME).ipa; \
			fi; \
		fi; \
	else \
		echo 'Device not supported for deploy recipe.'; \
	fi
	echo '[ShardLauncher v$(VERSION)] deploy - end'

package: payload
	echo '[ShardLauncher v$(VERSION)] package - start'
	if [ '$(TEAMID)' != '-1' ] && [ '$(SIGNING_TEAMID)' != '-1' ] && [ -f '$(PROVISIONING)' ] && [ '$(DETECTPLAT)' = 'Darwin' ]; then \
		printf '<?xml version="1.0" encoding="UTF-8"?>\n<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n<plist version="1.0">\n<dict>\n	<key>application-identifier</key>\n	<string>$(TEAMID).com.lanrhyme.shardlauncher</string>\n	<key>com.apple.developer.team-identifier</key>\n	<string>$(TEAMID)</string>\n	<key>get-task-allow</key>\n	<true/>\n	<key>keychain-access-groups</key>\n	<array>\n	<string>$(TEAMID).*</string>\n	<string>com.apple.token</string>\n	</array>\n</dict>\n</plist>' > entitlements.codesign.xml; \
		$(MAKE) codesign; \
	...
	echo '[ShardLauncher v$(VERSION)] package - end'
	
dsym: payload
	echo '[ShardLauncher v$(VERSION)] dsym - start'
	dsymutil --arch arm64 $(OUTPUTDIR)/Payload/ShardLauncher.app/ShardLauncher; \
	rm -rf $(OUTPUTDIR)/ShardLauncher.dSYM; \
	mv $(OUTPUTDIR)/Payload/ShardLauncher.app/ShardLauncher.dSYM $(OUTPUTDIR)/ShardLauncher.dSYM
	echo '[ShardLauncher v$(VERSION)] dsym - end'
	
codesign:
	echo '[ShardLauncher v$(VERSION)] codesign - start'
	cp '$(PROVISIONING)' $(OUTPUTDIR)/Payload/ShardLauncher.app/embedded.mobileprovision
	$(call METHOD_MACHO,$(OUTPUTDIR)/Payload/ShardLauncher.app,$(call METHOD_CODESIGN,$(SIGNING_TEAMID),$$file))
	$(call METHOD_MACHO,$(OUTPUTDIR)/java_runtimes,$(call METHOD_CODESIGN,$(SIGNING_TEAMID),$$file))
	echo '[ShardLauncher v$(VERSION)] codesign - end'
clean:
	echo '[ShardLauncher v$(VERSION)] clean - start'
	rm -rf $(WORKINGDIR)
	rm -rf JavaApp/build
	rm -rf $(OUTPUTDIR)
	echo '[ShardLauncher v$(VERSION)] clean - end'
